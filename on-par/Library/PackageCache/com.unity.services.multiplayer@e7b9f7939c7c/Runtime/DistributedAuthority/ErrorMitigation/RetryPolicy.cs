...
namespace Unity.Services.DistributedAuthority.ErrorMitigation
{
    ...
    internal class RetryPolicyProvider : IRetryPolicyProvider
    {
        ...
        public RetryPolicyProvider(IActionScheduler actionScheduler)
        {
        ...
        {
            return RetryPolicy<T>.ForOperation(m_ActionScheduler, operation);
        }
    ...
    internal class RetryPolicy<T> : IRetryPolicy<T>
    {
        ...
        private RetryPolicy(IActionScheduler actionScheduler, Func<int, Task<T>> createAsyncOp)
        {
            m_ActionScheduler = actionScheduler;
            CreateOperation = createAsyncOp;
        }

        private RetryPolicy(IActionScheduler actionScheduler, Func<Task<T>> createAsyncOp)
        {
            m_ActionScheduler = actionScheduler;
        ...
        /// Constructs a RetryPolicy based on the passed in operations type.
        /// </summary>
        /// <param name="actionScheduler">Scheduler for the retries.</param>
        ...
        public static RetryPolicy<T> ForOperation(IActionScheduler actionScheduler, Func<int, Task<T>> operation)
        {
            return new RetryPolicy<T>(actionScheduler, operation);
        }
        ...
        /// </param>
        /// <returns>A RetryPolicy{T}.</returns>
        public static RetryPolicy<T> ForOperation(IActionScheduler actionScheduler, Func<Task<T>> operation)
        {
            return new RetryPolicy<T>(actionScheduler, operation);
        }
        ...
        public async Task<T> RunAsync(RetryPolicyConfig retryPolicyConfig = null)
        {
            ...
            for (var attempt = 0; attempt <= retryPolicyConfig.MaxRetries; ++attempt)
            {
                ...
                m_ActionScheduler.ScheduleAction(() => completionSource.SetResult(null), (int)(delayTime));
