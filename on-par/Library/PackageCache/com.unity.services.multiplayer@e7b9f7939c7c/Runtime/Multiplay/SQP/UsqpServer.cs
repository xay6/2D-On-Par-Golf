...
namespace Unity.Ucg.Usqp
{
    internal class UsqpServer : IDisposable
    {
        ...
        public UsqpServer(IPEndPoint endpoint)
        {
            ...
            var socketError = m_Socket.SetupAndBind(ServerEndpoint.Address, ServerEndpoint.Port);
            if (socketError != SocketError.Success)
            {
                throw new SqpException("Failed to bind socket", new SocketException((int)socketError));
            }
            m_Random = new Random();
